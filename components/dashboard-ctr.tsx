...
function ChantierMeteoIcon({ meteo }: { meteo: string }) {
  switch (meteo) {
    case "soleil": return <Sun className="size-4 text-amber-500" />;
    case "nuageux": return <Cloud className="size-4 text-gray-500" />;
    case "orage": return <CloudLightning className="size-4 text-red-500" />;
    default: return <Sun className="size-4 text-amber-500" />;
  }
}

function TendanceIcon({ tendance }: { tendance: string }) {
  if (tendance === "hausse") return <ArrowUp className="size-3.5 text-green-600" />;
  return <ArrowDown className="size-3.5 text-red-600" />;
}
...
export function DashboardCTR({ data }: { data: CTRData }) {
  const budgetPct = data.budgetJHTotal > 0
    ? Math.round((data.budgetConsumed / data.budgetJHTotal) * 100)
    : 0;

  const fmtDate = (iso: string) => format(new Date(iso), "dd/MM/yyyy", { locale: fr });

  return (
    <div className="rounded-xl border-2 border-emerald-500/20 bg-card shadow-lg overflow-hidden print:shadow-none print:border">
      {/* Header band */}
      <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 px-6 py-4 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold tracking-tight">Dashboard CTR — proposition</h2>
            <p className="text-sm opacity-90">
              Période : Du {fmtDate(data.periodeDebut)} au {fmtDate(data.periodeFin)}
            </p>
          </div>
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-5 lg:grid-cols-9">
          ...
          <KpiCard
            ...
            value={`${data.adherencePlanning}%`}
            icon={CalendarCheck}
            color="text-violet-600"
          />
          <KpiCard
            label="Taux de staffing"
            value={`${data.staffingRate}%`}
            icon={Users}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="px-3 py-2 text-center font-medium">Avt</th>
                  <th className="px-3 py-2 text-center font-medium">SPI</th>
                  <th className="px-3 py-2 text-center font-medium">Météo</th>
              ...
              <tbody className="divide-y">
                ...
                {data.topChantiers.map((c, i) => (
                  <tr key={i} className="hover:bg-muted/30">
                    <td className="px-3 py-2">
                      <span className="font-medium">{c.code}</span>
                      <span className="text-muted-foreground ml-1">— {c.nom}</span>
                    </td>
                    <td className="px-3 py-2">
                      <Badge variant="outline" className="text-[10px]">{c.phase}</Badge>
                    </td>
                    <td className="px-3 py-2 text-center font-medium">{c.avancement}%</td>
                    <td className="px-3 py-2 text-center">
                      <span className={c.spi >= 1 ? "text-green-600 font-medium" : "text-red-600 font-medium"}>
                        {c.spi.toFixed(2)}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-center">
                      <ChantierMeteoIcon meteo={c.meteo} />
                    </td>
                    <td className="px-3 py-2 text-center">
        ...
        <div className="rounded-lg border p-4">
          ...
          <div className="h-5 w-full rounded-full bg-muted overflow-hidden">
            <div
              className="h-full rounded-full transition-all bg-gradient-to-r from-emerald-600 to-emerald-400"
